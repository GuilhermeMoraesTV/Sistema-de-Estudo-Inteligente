...
import { checkTextSizeForIA, createSubmitGuard, LIMITS } from "../lib/security";
...
const Upload = () => {
  ...
  const extrairTextoPDF = async (file: File): Promise<string> => {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      textoCompleto += (content.items as Array<{ str?: string }>)
  ...
  const processarENavegar = async (textoFinal: string, nomeArq?: string) => {
    ...
    if (!textoFinal || textoFinal.trim().length < 10) {
      setErro("O conteúdo é muito curto para processar.");
      return;
    }

    // Trunca se exceder o limite absoluto (proteção de segurança)
    const textoProcessado = textoFinal.slice(0, LIMITS.TEXTO_ORIGINAL_MAX);
    ...
    try {
      ...
      const mapa = await mapearAssuntos(textoProcessado, nomeArq);
    ...
    await submitGuard.current(async () => {
      ...
      try {
        ...
        if (!textoExtraido || textoExtraido.trim().length < 10) {
          setErro("Não foi possível extrair texto legível deste PDF. Verifique se o arquivo não é uma imagem escaneada.");
          setFase("idle");
          setModalAberto(true);
          return;
        }
        ...
        setErro("Erro ao processar o PDF. Tente com outro arquivo.");
        setFase("idle");
        setModalAberto(true);
      } finally {
        setCarregando(false);
  ...
  const handleTextoChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    // Verifica tamanho e exibe aviso ao usuário
    const check = checkTextSizeForIA(val);
    setTextWarning(check.warning);
  };
              ...
              <div className="p-8">
                ...
                <div className="relative">
                  ...
                  <div className="flex items-center justify-between mt-2 px-1 flex-wrap gap-1">
                    <span className="text-[11px] text-muted-foreground">{charCount.toLocaleString("pt-BR")} caracteres</span>
                    <div className="flex items-center gap-2">
                      ...
                      {charCount > LIMITS.TEXTO_ORIGINAL_MAX && (
                        <span className="text-[11px] text-red-400">
                          Texto truncado em {LIMITS.TEXTO_ORIGINAL_MAX.toLocaleString("pt-BR")} chars
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {/* Aviso de texto grande */}
                {textWarning && (
                  <div className="mt-3 flex items-start gap-2 bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-3 py-2.5 animate-fade-in">
                    <span className="text-yellow-400 text-sm shrink-0">⚠️</span>
                    <p className="text-xs text-yellow-400/90 leading-relaxed">{textWarning}</p>
                  </div>
                )}
                ...
                <button
                  ...
                  disabled={texto.trim().length === 0 || carregando}
